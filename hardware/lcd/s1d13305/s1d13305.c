...
/* for menuconfig */
#define S1D13305_IF_PAR 1
#define S1D13305_IF_I2C 2

#if S1D13305_INTERFACE == S1D13305_IF_I2C
#define S1D13305_I2C
#endif

#ifdef S1D13305_I2C
#define I2C_WRITESIGNALS(x) i2c_pca9555_writeOutPortH(0,x)
#define I2C_READSIGNALS(x) i2c_pca9555_readOutPortH(0,x)
#define I2C_WRITEDATA(x) i2c_pca9555_writeOutPortL(0, x)
#define I2C_READDATA(x) i2c_pca9555_readInPortL(0,x)
#define I2C_WRITEBOTH(data,signals) i2c_pca9555_writeOutPort(0,(uint16_t)((((uint16_t)signals << 8)&0xFF00) | (data & 0x00FF)))

#define S1D13305_A0 1
#define S1D13305_CS 2
#define S1D13305_WR 4
#define S1D13305_RD 8
#define S1D13305_RES 16
#define LCD_BACKLIGHT 32
#define S1D13305_PSR 64
#endif
...
/* local variables */
static bool displayOn_gb = 0; /* 1=display on, 0=display off */
static uint8_t signals_gui8 = 0;
...
void lcd_init(void)
{
...
#ifdef S1D13305_I2C
  i2c_pca9555_writeOutPort(0, 0xFFFF);
  //Set all pins to 1
  i2c_pca9555_setDDR(0, 0);
  // Configure all pins as outputs
  i2c_pca9555_setPolarity(0, 0);
  //Low level is logic 0
  I2C_WRITEDATA(0);

  signals_gui8 = S1D13305_WR | S1D13305_RD | S1D13305_A0 | LCD_BACKLIGHT;
  I2C_WRITESIGNALS(signals_gui8);

  _delay_ms(1);

  signals_gui8 |= S1D13305_RES;
  I2C_WRITESIGNALS(signals_gui8);
  _delay_ms(3);

  signals_gui8 |= S1D13305_CS;
  I2C_WRITESIGNALS(signals_gui8);
  _delay_ms(1);
  signals_gui8 &= ~S1D13305_CS;
  I2C_WRITESIGNALS(signals_gui8);
  ...
  signals_gui8 |= S1D13305_CS;
  signals_gui8 &= ~S1D13305_A0;
  I2C_WRITESIGNALS(signals_gui8);

  signals_gui8 = S1D13305_WR | S1D13305_RD | S1D13305_CS | S1D13305_A0
      | LCD_BACKLIGHT | S1D13305_RES;
  I2C_WRITESIGNALS(signals_gui8);
  ...
  lcd_setLayerModes(LCD_CURSOR_OFF,
      LCD_LAYER1_ON | LCD_LAYER2_ON | LCD_LAYER3_ON);
  ...
  lcd_writeTextPos(10, 1, "Hallo Welt!", 11);
}
 ...
 */
void lcd_writeTextPos(uint8_t x_ui8, uint8_t y_ui8, uint8_t* text_pui8,
    uint16_t len_ui16)
{
...
{
#ifdef S1D13305_I2C
  signals_gui8 &= ~(S1D13305_A0 | S1D13305_CS | S1D13305_WR);
  I2C_WRITEBOTH(data_ui8, signals_gui8);

  signals_gui8 |= S1D13305_CS | S1D13305_A0 | S1D13305_WR;
  I2C_WRITESIGNALS(signals_gui8);
#else
...
uint8_t lcd_readData()
{
  ...
  I2C_WRITEDATA(0xFF);
  i2c_pca9555_setDDRL(0, 0xFF); /* configure LCD data port as input */

  signals_gui8 &= ~(S1D13305_CS|S1D13305_RD);
  I2C_WRITESIGNALS(signals_gui8);

  _delay_us(1);

  I2C_READDATA(&result_ui8);
  signals_gui8 |= S1D13305_RD|S1D13305_CS;
  I2C_WRITESIGNALS(signals_gui8);

  i2c_pca9555_setDDRL(0, 0); /* configure LCD data port as output */
#else
...
{
#ifdef S1D13305_I2C
  signals_gui8 &= ~(S1D13305_CS | S1D13305_WR);
  I2C_WRITEBOTH(cmd_ui8, signals_gui8);
